<script lang="ts">
  ...
  const stream = new Stream({
    system:
      "You are a helpful assistant. Keep responses concise (2-3 sentences max).",
    model: "gpt",
  });
  ...
  $effect(() => {
    if (stream.done && stream.text) {
      if (regeneratingNodeId) {
        chat.regenerate(regeneratingNodeId, {
          role: "assistant",
          content: stream.text,
        });
        regeneratingNodeId = null;
...
<div class="container">
        ...
        {#if chat.hasAlternatives(nodeId)}
          <div class="branch-nav">
            ...
            <span class="branch-index">
              {chat.alternativeIndex(nodeId) + 1}/{chat.alternativeCount(
                nodeId,
              )}
            </span>
            <button
              onclick={() => chat.nextAlternative(nodeId)}
              disabled={chat.alternativeIndex(nodeId) ===
                chat.alternativeCount(nodeId) - 1}
            >
              &rarr;
            </button>
          </div>
        ...
        {:else}
          ...
          <div class="actions">
            {#if msg.role === "user"}
              <button
                class="action-btn"
                onclick={() => startEdit(nodeId, msg.content)}
              >
                Edit
              </button>
            {:else}
              <button
                class="action-btn"
                onclick={() => handleRegenerate(nodeId)}
              >
                Regenerate
              </button>
            {/if}
  ...
  <form class="input-form" onsubmit={handleSubmit}>
    ...
    />
    {#if stream.loading}
      <button type="button" class="stop-btn" onclick={() => stream.abort()}
        >Stop</button
      >
    {:else}
