...
describe('createStreamHandler (SolidStart)', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	});
	...
	it('handles /api/__aibind__/stream requests', async () => {
		...
		const response = await handle(makeRequest('/api/__aibind__/stream', { prompt: 'hello', system: 'be nice' }));
	...
	it('handles /api/__aibind__/structured requests', async () => {
		const handle = createStreamHandler({ model: 'test-model' });
		await handle(makeRequest('/api/__aibind__/structured', { prompt: 'analyze this' }));
	...
	it('returns 400 for missing prompt', async () => {
		const handle = createStreamHandler({ model: 'test-model' });
		const response = await handle(makeRequest('/api/__aibind__/stream', { prompt: '' }));
	...
	it('resolves named model from request body', async () => {
		...
		await handle(makeRequest('/api/__aibind__/stream', { prompt: 'hello', model: 'fast' }));
	...
	it('returns 400 for unknown model key', async () => {
		...
		const response = await handle(makeRequest('/api/__aibind__/stream', { prompt: 'hello', model: 'unknown' }));
	...
	it('returns error when no model configured', async () => {
		const handle = createStreamHandler({});
		const response = await handle(makeRequest('/api/__aibind__/stream', { prompt: 'hello' }));
	...
	it('returns 404 for GET requests', async () => {
		const handle = createStreamHandler({ model: 'test-model' });
		const response = await handle(new Request('http://localhost/api/__aibind__/stream', { method: 'GET' }));
	...
	it('handles structured endpoint with schema in body', async () => {
		...
		await handle(makeRequest('/api/__aibind__/structured', { prompt: 'extract', schema }));
