...
describe('useStructuredStream (SolidStart wrapper)', () => {
	...
	it('uses default endpoint /api/__aibind__/structured', async () => {
		...
		const { send } = useStructuredStream({ schema: schema as never, fetch: mockFetch });
		send('prompt');
		await flushPromises();
	...
	it('allows overriding endpoint', async () => {
		...
		const { send } = useStructuredStream({ schema: schema as never, fetch: mockFetch, endpoint: '/custom/structured' });
		send('prompt');
		await flushPromises();
	...
	it('validates and sets data', async () => {
		...
		const { data, send } = useStructuredStream({ schema: schema as never, fetch: mockFetch });
		send('prompt');
		await flushPromises();
