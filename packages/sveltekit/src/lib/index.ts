...
export type { SendOptions, DeepPartial, LanguageModel, StreamOptions, StructuredStreamOptions } from '@aibind/svelte';
...
export class Stream<M extends string = string> extends BaseStream<M> {
	constructor(options: Partial<Pick<StreamOptions<M>, 'endpoint'>> & Omit<StreamOptions<M>, 'endpoint'> = {} as any) {
 ...
 */
export class StructuredStream<M extends string, T> extends BaseStructuredStream<M, T> {
	constructor(options: Partial<Pick<StructuredStreamOptions<T, M>, 'endpoint'>> & Omit<StructuredStreamOptions<T, M>, 'endpoint'>) {
