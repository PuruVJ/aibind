...
describe('createStreamHandler', () => {
	...
	it('handles /api/__aibind__/stream requests', async () => {
		...
		const event = makeEvent('/api/__aibind__/stream', { prompt: 'hello', system: 'be nice' });
	...
	it('handles /api/__aibind__/structured requests', async () => {
		const handle = createStreamHandler({ model: 'test-model' });
		const event = makeEvent('/api/__aibind__/structured', { prompt: 'analyze this' });
	...
	it('resolves named model from request body', async () => {
		...
		const event = makeEvent('/api/__aibind__/stream', { prompt: 'hello', model: 'fast' });
	...
	it('throws for unknown model key', async () => {
		...
		const event = makeEvent('/api/__aibind__/stream', { prompt: 'hello', model: 'unknown' });
	...
	it('falls through for GET requests', async () => {
		...
		const event = {
			...
			request: new Request('http://localhost/api/__aibind__/stream', { method: 'GET' })
		};
	...
	it('handles structured endpoint with schema in body', async () => {
		...
		const event = makeEvent('/api/__aibind__/structured', { prompt: 'extract', schema });
