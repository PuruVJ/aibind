...
describe('useAgent', () => {
	...
	it('send() adds user message and POSTs', async () => {
		...
		const { send, messages } = useAgent({ endpoint: ENDPOINT, fetch: mockFetch });
		send('hello');
		await flushPromises();
	...
	it('send() streams response into assistant message', async () => {
		...
		const { send, messages, status } = useAgent({ endpoint: ENDPOINT, fetch: mockFetch });
		send('greet me');
		await flushPromises();
	...
	it('stop() aborts request and sets status idle', async () => {
		...
		const { send, stop, status } = useAgent({ endpoint: ENDPOINT, fetch: mockFetch });
		send('prompt');
		stop();
		await flushPromises();
	...
	it('approve() clears pendingApproval', () => {
		const { approve, pendingApproval, status } = useAgent({ endpoint: ENDPOINT });
	...
	it('network error sets error state and calls onError', async () => {
		...
		const { send, status, error } = useAgent({ endpoint: ENDPOINT, fetch: mockFetch, onError });
		send('prompt');
		await flushPromises();
	...
	it('calls onMessage when assistant responds', async () => {
		...
		const { send } = useAgent({ endpoint: ENDPOINT, fetch: mockFetch, onMessage });
		send('hello');
		await flushPromises();
