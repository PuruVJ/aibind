...
describe('ReactiveMessageTree (Solid)', () => {
	...
	it('append creates root on empty tree and updates reactives', () => {
		createRoot((dispose) => {
			...
			expect(tree.activePath().messages).toEqual([{ role: 'user', content: 'hello' }]);
			dispose();
		});
	});
	...
	it('serialize and deserialize round-trip', () => {
		createRoot((dispose) => {
			const tree = new ReactiveMessageTree<Msg>(freshConfig());
			const m1 = tree.append({ role: 'user', content: 'hello' }, { model: 'gpt-4' });
			tree.append({ role: 'assistant', content: 'hi' });
			...
			expect(restored.activePath().messages).toEqual(tree.activePath().messages);
