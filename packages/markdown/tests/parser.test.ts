...
describe('inline code', () => {
	...
	it('double backtick inline code', () => {
		assertMarkdown('``code with ` backtick``\n', '<p><code>code with ` backtick</code></p>\n');
	});
...
describe('code blocks', () => {
	...
	it('unclosed code block at end of input is closed by parser', () => {
		const html = parse('```js\nconst x = 1;');
		expect(html).toBe('<pre><code class="language-js">const x = 1;\n</code></pre>\n');
	});
...
describe('images', () => {
	it('basic image', () => {
		assertMarkdown('![alt text](image.png)\n',
			'<p><img src="image.png" alt="alt text"></p>\n');
	});

	it('image with URL', () => {
		assertMarkdown('![logo](https://example.com/logo.png)\n',
			'<p><img src="https://example.com/logo.png" alt="logo"></p>\n');
	});

	it('image in text', () => {
		assertMarkdown('See ![pic](a.png) here\n',
...
describe('html escaping', () => {
	it('escapes < and >', () => {
		assertMarkdown('<script>alert("xss")</script>\n',
...
describe('inline edge cases', () => {
	...
	it('link with bold text: [**bold**](url)', () => {
		assertMarkdown('[**bold**](http://x.com)\n',
...
describe('block edge cases', () => {
	...
	it('paragraph with only a link', () => {
		assertMarkdown('[click](http://x.com)\n',
...
describe('unicode and special content', () => {
	...
	it('link with unicode text', () => {
		assertMarkdown('[日本語](https://example.com)\n',
...
describe('security edge cases', () => {
	it('script tag in heading', () => {
		assertMarkdown('# <script>alert(1)</script>\n',
	...
	it('nested HTML tags', () => {
		assertMarkdown('<div><script>alert(1)</script></div>\n',
...
describe('streaming behavior', () => {
	...
	it('split in middle of code fence', () => {
		...
		parser.write('``\n');
		parser.end();
		expect(renderer.html).toBe('<pre><code class="language-js">const x = 1;\n</code></pre>\n');
	});
	...
	it('split in middle of link syntax', () => {
		...
		parser.write('.com)\n');
		parser.end();
		expect(renderer.html).toBe('<p><a href="http://example.com">link</a></p>\n');
	});
