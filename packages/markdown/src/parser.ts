...
const T_PARAGRAPH: Token = Object.freeze({ type: 'paragraph', attrs: EMPTY_ATTRS });
const T_STRONG: Token = Object.freeze({ type: 'strong', attrs: EMPTY_ATTRS });
const T_EMPHASIS: Token = Object.freeze({ type: 'emphasis', attrs: EMPTY_ATTRS });
const T_STRIKETHROUGH: Token = Object.freeze({ type: 'strikethrough', attrs: EMPTY_ATTRS });
const T_CODE: Token = Object.freeze({ type: 'code', attrs: EMPTY_ATTRS });
const T_LIST_ITEM: Token = Object.freeze({ type: 'list_item', attrs: EMPTY_ATTRS });
const T_HR: Token = Object.freeze({ type: 'hr', attrs: EMPTY_ATTRS });
const T_BLOCKQUOTE: Token = Object.freeze({ type: 'blockquote', attrs: EMPTY_ATTRS });
const T_ORDERED_LIST: Token = Object.freeze({ type: 'ordered_list', attrs: EMPTY_ATTRS });
const T_UNORDERED_LIST: Token = Object.freeze({ type: 'unordered_list', attrs: EMPTY_ATTRS });
const T_LINE_BREAK: Token = Object.freeze({ type: 'line_break', attrs: EMPTY_ATTRS });

// Heading tokens cached by level (1-6)
const T_HEADING: Token[] = [];
for (let i = 0; i <= 6; i++) {
	T_HEADING[i] = Object.freeze({ type: 'heading', attrs: Object.freeze({ level: String(i) }) });
}

// Reusable list result object (mutated in place to avoid allocation)
const _list = { type: 'unordered_list' as 'ordered_list' | 'unordered_list', indent: 0, contentStart: 0 };
...
export class StreamParser {
	...
	/** Signal end of input. Flushes any remaining content. */
		...
		if (this.#inCodeBlock) {
			this.#renderer.close({ type: 'code_block', attrs: { lang: this.#codeLanguage } });
	...
	#closeAllLists() {
		while (this.#listStack.length > 0) {
			...
			this.#renderer.close(listType === 'ordered_list' ? T_ORDERED_LIST : T_UNORDERED_LIST);
		}
		this.#listIndents.length = 0;
	}
	...
	#findClosingSingle(text: string, char: string, start: number, end: number): number {
	...
	#detectListItem(line: string, lineLen: number): boolean {
		...
		if ((ch === CHAR_DASH || ch === CHAR_STAR || ch === CHAR_PLUS) &&
			indent + 1 < lineLen && line.charCodeAt(indent + 1) === CHAR_SPACE) {
		...
		if (ch >= CHAR_0 && ch <= CHAR_9) {
			let numEnd = indent + 1;
			while (numEnd < lineLen && line.charCodeAt(numEnd) >= CHAR_0 && line.charCodeAt(numEnd) <= CHAR_9) numEnd++;
			if (numEnd < lineLen) {
				const after = line.charCodeAt(numEnd);
				if ((after === CHAR_DOT || after === CHAR_PAREN_CLOSE) &&
					numEnd + 1 < lineLen && line.charCodeAt(numEnd + 1) === CHAR_SPACE) {
	...
	#detectHeading(line: string, lineLen: number): number {
		let level = 0;
		while (level < lineLen && level < 6 && line.charCodeAt(level) === CHAR_HASH) level++;
		if (level > 0 && level <= 6 && level < lineLen && line.charCodeAt(level) === CHAR_SPACE) {
			return level;
		}
		return 0;
	}
	...
	#isHorizontalRule(line: string, lineLen: number, firstCode: number): boolean {
		if (firstCode !== CHAR_STAR && firstCode !== CHAR_DASH && firstCode !== CHAR_UNDERSCORE) return false;
		let count = 0;
	...
	#processLine(line: string) {
		...
		if (this.#inCodeBlock) {
			...
			let trimStart = 0;
			while (trimStart < lineLen && line.charCodeAt(trimStart) === CHAR_SPACE) trimStart++;
			const fenceLen = this.#codeFence.length;
			let match = trimStart + fenceLen <= lineLen;
			if (match) {
				for (let k = 0; k < fenceLen; k++) {
					if (line.charCodeAt(trimStart + k) !== this.#codeFence.charCodeAt(k)) {
						match = false;
						break;
					}
				}
		...
		if (maybeBlock) {
			// Code fence
			if (firstCode === CHAR_BACKTICK || firstCode === CHAR_TILDE || firstCode === CHAR_SPACE) {
				const fence = this.#detectCodeFence(line, lineLen);
				if (fence) {
					this.#closeOpenBlocks();
					this.#inCodeBlock = true;
					this.#codeFence = fence;
					this.#codeLanguage = line.substring(line.indexOf(fence) + fence.length).trim();
			...
			if (firstCode === CHAR_GT) {
				const contentStart = (lineLen > 1 && line.charCodeAt(1) === CHAR_SPACE) ? 2 : 1;
				if (!this.#inBlockquote) {
					this.#closeOpenBlocks();
			...
			if (this.#detectListItem(line, lineLen)) {
				...
				while (this.#listStack.length > 0 && this.#listIndents[this.#listIndents.length - 1] > indent) {
					r.close(T_LIST_ITEM);
				...
				if (this.#listStack.length === 0 || this.#listIndents[this.#listIndents.length - 1] < indent) {
		...
		if (lineLen <= 4) {
			...
			for (let j = 0; j < lineLen; j++) {
				const c = line.charCodeAt(j);
				if (c !== CHAR_SPACE && c !== CHAR_TAB) { allSpace = false; break; }
			}
			if (allSpace) {
				if (this.#listStack.length > 0) this.#closeAllLists();
				this.#closeParagraph();
		...
		if (lineLen >= 2 && line.charCodeAt(lineLen - 1) === CHAR_SPACE && line.charCodeAt(lineLen - 2) === CHAR_SPACE) {
			contentEnd = lineLen - 1;
			while (contentEnd >= 0 && line.charCodeAt(contentEnd) === CHAR_SPACE) contentEnd--;
			contentEnd++;
