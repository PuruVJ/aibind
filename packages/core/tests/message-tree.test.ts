...
describe("MessageTree", () => {
  ...
  describe("addChild", () => {
    ...
    it("throws on invalid parentId", () => {
      const tree = new MessageTree<Msg>();
      expect(() =>
        tree.addChild("nonexistent", { role: "user", content: "" }),
  ...
  describe("branch", () => {
    ...
    it("throws on invalid parentId", () => {
      const tree = new MessageTree<Msg>();
      expect(() =>
        tree.branch("nonexistent", { role: "user", content: "" }),
      ).toThrow("does not exist");
    });
  });
  ...
  describe("serialization", () => {
    ...
    it("deserialize() reconstructs identical tree", () => {
      ...
      expect(restored.getActivePath().messages).toEqual(
  ...
  describe("edge cases", () => {
    ...
    it("wide tree (50 siblings): navigation works", () => {
      ...
      for (let i = 0; i < 50; i++) {
        childIds.push(
          tree.addChild(root, { role: "assistant", content: `child-${i}` }),
        );
      }
