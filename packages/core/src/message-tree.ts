...
export class MessageTree<M> {
	 ...
	 */
	static deserialize<M>(data: SerializedTree<M>, config?: TreeConfig): MessageTree<M> {
		if (data.version !== 1) {
		...
		for (const [id, node] of nodeEntries) {
			if (node.parentId !== null && !tree.#nodes.has(node.parentId)) {
				throw new Error(`MessageTree: node "${id}" references missing parent "${node.parentId}"`);
			}
			for (const childId of node.children) {
				if (!tree.#nodes.has(childId)) {
					throw new Error(`MessageTree: node "${id}" references missing child "${childId}"`);
				}
				const child = tree.#nodes.get(childId)!;
				if (child.parentId !== id) {
					throw new Error(`MessageTree: child "${childId}" parentId does not match parent "${id}"`);
				}
			}
		}
		...
		for (const rootId of data.rootIds) {
			if (!tree.#nodes.has(rootId)) {
				throw new Error(`MessageTree: rootIds references missing node "${rootId}"`);
			}
		...
		if (data.activeLeafId !== null && !tree.#nodes.has(data.activeLeafId)) {
			throw new Error(`MessageTree: activeLeafId "${data.activeLeafId}" does not exist`);
		}
		...
		if (this.#activeLeafId !== null && toRemove.has(this.#activeLeafId)) {
			if (node.parentId !== null) {
				...
				if (parent.children.length > 0) {
					this.#activeLeafId = this.#followFirstChildToLeaf(parent.children[parent.children.length - 1]);
				} else {
					this.#activeLeafId = node.parentId;
				}
			} else if (this.#rootIds.length > 0) {
				this.#activeLeafId = this.#followFirstChildToLeaf(this.#rootIds[this.#rootIds.length - 1]);
			} else {
				this.#activeLeafId = null;
			}
		}
	...
	#createNode(
		...
		const node: TreeNode<M> = {
			...
			parentId,
			children: [],
			metadata: Object.freeze({ ...metadata }) as Readonly<Record<string, unknown>>,
	...
	#getSiblingIds(nodeId: string): { siblings: readonly string[]; index: number } {
		const node = this.#nodes.get(nodeId);
		if (!node) {
			throw new Error(`MessageTree: node "${nodeId}" does not exist`);
		}
