/**
 * Framework-agnostic transport base for all stream controllers.
 *
 * Handles fetch, SSE auto-detection, reconnect, stop/resume, abort.
 * Does NOT know about text accumulation, diffs, artifacts, or structured data —
 * those concerns live in the subclasses.
 */
...
// --- Base callbacks (transport-level only) ---

export interface BaseStreamCallbacks {
...
export interface BaseStreamControllerOptions {
  model?: string;
  system?: string;
  endpoint: string;
...
export class BaseStreamController {
  protected _opts: BaseStreamControllerOptions;
  protected _cb: BaseStreamCallbacks;
  protected _done = false;
  protected _status: StreamStatus = "idle";
  protected _currentModel: string | undefined = undefined;
  ...
  /** Called for each plain text data chunk (SSE or raw body). */
  protected _processChunk(_chunk: string): void {}

  /** Called when the stream finishes (after `done` event or body end). */
  protected async _finalize(): Promise<void> {}

  /** Called at the start of each `send()` to clear prior state. */
  protected _resetState(): void {}

  /**
   * Called for named SSE events not handled by the base class.
   * Return `true` to consume the event; `false` to fall through to `_processChunk`.
   */
  protected _handleNamedEvent(_event: string, _data: string): boolean {
    return false;
  }

  /** Called after state changes that should be broadcast cross-tab. */
  protected _postBroadcast(_error?: Error): void {}

  /** Called after `_finalize()` completes — for diff computation etc. */
  protected _postFinalize(): void {}
  ...
  private async _consumeSSE(
    ...
    for await (const msg of SSE.consume(response)) {
      ...
      // Delegate named events to subclasses before falling through
      if (msg.event && this._handleNamedEvent(msg.event, msg.data)) continue;

      // Regular text data chunk
