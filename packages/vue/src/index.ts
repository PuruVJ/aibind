	...
	async function resolveSchema(): Promise<Record<string, unknown> | null> {
		...
		if (std?.vendor === 'valibot') {
			try {
				...
				throw new Error('@aibind/vue: Valibot schema detected but "@valibot/to-json-schema" is not installed. Install it or switch to a schema library with built-in JSON Schema support.');
			}
		}
		...
		if (std?.vendor === 'zod') {
			try {
				...
				throw new Error('@aibind/vue: Zod schema detected but "zod/v4" is not available. Use `import { z } from "zod/v4"` to create schemas, or install a newer version of zod.');
			}
		}
