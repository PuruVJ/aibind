...
export const StreamMarkdown: ReturnType<typeof defineComponent> = defineComponent({
	name: 'StreamMarkdown',
	props: {
	...
	setup(props) {
		...
		const html = computed(() => {
			const input = props.streaming ? MarkdownRecovery.recover(props.text) : props.text;
			renderer.reset();
