...
describe('useStructuredStream', () => {
	...
	it('resolves schema from ~standard.jsonSchema', async () => {
		...
		const { send } = useStructuredStream({ schema: schema as never, fetch: mockFetch, endpoint: '/api/structured' });
		send('prompt');
		await flushPromises();
	...
	it('streams raw JSON and populates .partial', async () => {
		...
		const mockFetch = vi.fn().mockResolvedValue(createMockResponse(['{"name":', '"Bob"}']));

		const { partial, send } = useStructuredStream({ schema: schema as never, fetch: mockFetch, endpoint: '/api/structured' });
		send('prompt');
		await flushPromises();
	...
	it('final JSON validates and sets .data', async () => {
		...
		const { data, send } = useStructuredStream({ schema: schema as never, fetch: mockFetch, endpoint: '/api/structured' });
		send('prompt');
		await flushPromises();
	...
	it('validation failure sets .error', async () => {
		...
		const { error, send } = useStructuredStream({ schema: schema as never, fetch: mockFetch, endpoint: '/api/structured' });
		send('prompt');
		await flushPromises();
	...
	it('calls onFinish with validated data', async () => {
		...
		const { send } = useStructuredStream({ schema: schema as never, fetch: mockFetch, endpoint: '/api/structured', onFinish });
		send('prompt');
		await flushPromises();
